...
class Handlers:
	...
	async def handle_map_search(self, request):
		# Вытащил весь блок команд из-под обработки исключений (из-за нее было невозможно отлаживать)
		user_api_request = request.match_info
	...
	async def handle_show_interactives(self, request):
		...
		try:
			...
			return web.json_response(requested_interactives)
		except KeyError:
			print('Wrong JSON Key')
			raise web.HTTPBadRequest(reason='wrong_json_key')
